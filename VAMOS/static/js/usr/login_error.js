...
// Classe principale pour gérer le formulaire de connexion
class LoginForm {
    constructor() {
        ...
        try {
            ...
            if (data.success) {
                ...
                if (data.error === 'Incorrect password') {
                    this.showError(this.passwordInput, this.passwordErrorMessage, data.error);
                } else if (data.error === 'User not found') {
                    this.showError(this.usernameInput, this.usernameErrorMessage, data.error);
                }
            }
        } catch (error) {
            console.error("Erreur AJAX :", error);
            this.showError(this.form.querySelector('.general-error'), 'Une erreur inattendue est survenue');
        } finally {
